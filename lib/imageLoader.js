...
// Next.js image optimizer widths — must be a subset of next.config.js
// deviceSizes + imageSizes, otherwise /_next/image returns HTTP 400.
const NEXT_ALLOWED_WIDTHS = [16, 32, 48, 64, 96, 128, 256, 384, 640, 750, 828, 1080, 1200, 1920, 2048, 3840];

// Rewrite a remote (CloudFront) image URL to go through Next.js's built-in
// image optimizer, so the browser downloads a small, resized AVIF/WebP
// thumbnail instead of the multi-MB full-resolution original. Use this for
// grid tiles / previews only — keep the raw URL for the lightbox, downloads
// and PDF export, which need full resolution.
//
// data:, blob:, already-optimized and placeholder URLs are returned untouched.
const optimizedSrc = (url, width = 640, quality = 75) => {
  ...
  const w = NEXT_ALLOWED_WIDTHS.includes(width) ? width : 640;
...
export { imageCache, preloadImage, preloadImagesBatch, preloadAllImages, optimizedSrc };