...
const imageCache = new ImageCache(1000);
...
const preloadImage = (url, priority = false, retryAttempt = 0, maxRetries = 3) => {
  return new Promise((resolve, reject) => {
    ...
    // Create a promise for this image with timeout for large images
    const imagePromise = new Promise((resolveLoad, rejectLoad) => {
      const timeout = priority ? 30000 : 60000; // 30s for priority, 60s for others (for large 2.5MB images)
      ...
      if (useFetch) {
        ...
        fetch(url, {
          ...
          signal: AbortSignal.timeout(timeout - 1000) // Abort slightly before timeout
        })
        .then(response => {
...
const preloadImagesBatch = async (urls, concurrency = 6, priority = false, onProgress) => {
  const results = [];
  ...
  for (let i = 0; i < urls.length; i += concurrency) {
    ...
    // Smaller delay for priority batches
    if (i + concurrency < urls.length) {
      await new Promise(resolve => setTimeout(resolve, priority ? 5 : 10));
    }
  }
  return results;
};
...
const preloadAllImages = async (wallpapers, onProgress) => {
  ...
  const results = await preloadImagesBatch(
    imageUrls,
    8, // Higher concurrency for faster loading
    true, // High priority
    onProgress
  );
  ...
  if (failed.length > 0) {
    ...
    await preloadImagesBatch(failed, 4, false, onProgress);
  }
